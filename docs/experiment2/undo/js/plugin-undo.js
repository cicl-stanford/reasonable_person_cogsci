var jsPsychUndo = (function (jspsych) {
  "use strict";
  ...
  class UndoPlugin {
    ...
    trial(display_element, trial) {
      ...
      html += '<div id="question">' +
        '<span>In order to undo the outcome,  which of these about ' +
        trial.name + ' would you change?<br><b>That ' + trial.name + '...</b> </span>' +
        // '<br> <div id="reminder" style="padding: 15px; color: red; margin-top: -8px; margin-bottom: -10px;">' +
        // ' <b>Please click on the ones that you would change.</b></div>' +
        '</div>';
      ...
      // show features
      html += '<div id="features" style="height: 400px; overflow-x: hidden; overflow-y: scroll; padding-bottom: 30px;">';
      for (var i = 0; i < questions.length; i++) {
        html += '<div class="feature-statement">' +
          '<button class="feature-opt-btn" data-order="' + question_order[i] + '" data-label="' + questions[question_order[i]] + '"> ...' + questions[question_order[i]] + '.</button>' +
          '</div>';
      }
      html += '</div>';
      ...
      html += '<p id="reminder"> Please click on' +
        ' all the ones you would change. </p>';
      // add submit button
      html += '<button id="undo-next" class="jspsych-btn" style="display: block;' +
      ...
      document.querySelector("#undo-next").addEventListener("click", () => {
        end_trial();
      });
